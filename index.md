...
**一定の長さを超えた場合の動作は保証していません。** 文や文節の区切りごとに変換することをおすすめします。
...
<div class="faq">
  <details>
    ...
    <p>使えます。設定画面で起動用のキーを「右Ctrl キー」に変更してください。変換キーは日本語配列（106/109 キー）にあるキーで英語配列（US / ANSI）には存在しませんが、右Ctrl キーはどちらの配列にもあります。右Ctrl キーを単独で押したときだけ働くため、Ctrl+C などの組み合わせ操作はこれまでどおり使えます。</p>
  </details>
  <details>
  ...
  "mainEntity": [
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "使えます。設定画面で起動用のキーを「右Ctrl キー」に変更してください。変換キーは日本語配列（106/109 キー）にあるキーで英語配列（US / ANSI）には存在しませんが、右Ctrl キーはどちらの配列にもあります。右Ctrl キーを単独で押したときだけ働くため、Ctrl+C などの組み合わせ操作はこれまでどおり使えます。"
      }
    },
    {
      "@type": "Question",
