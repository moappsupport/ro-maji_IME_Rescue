...
<div class="hero">
  ...
  <p class="hero-lead-note">（設定で右Ctrl キーにも変更できます。Microsoft IME / Google 日本語入力で動作確認済み）</p>
  ...
  <div class="hero-showcase">
    ...
    <ul class="cards">
      <li><b>変換キーひとつで完結</b><span>変換キーを押すだけ。覚えるショートカットはありません。英語配列のキーボードでは右Ctrl キーに変更できます。</span></li>
...
カーソル直前から続く**半角の英数字・記号**をまとめて変換します。範囲を選択する必要はありません。半角スペースや日本語など、それ以外の文字が現れた時点で止まります。
...
- **設定画面に「安定重視 / 標準 / 最速」を追加しました**
  ...
  待機時間を個別に調整する前にこの 3 つから選べます。

### v1.0.3

- **大文字表記の英単語がそのまま残るようになりました**
...
<div class="settings">
  ...
  <ul class="settings-list">
    ...
    <li><b>取得範囲</b><span>カーソル位置からどこまで遡って読み取るかを選びます。行頭まで／直前2行まで／文書先頭まで（既定）。「行頭まで」は画面の折り返しで途中までしか取れないアプリがあるため、既定は最も確実な「文書先頭まで」です。広く取っても<b>変換される範囲は変わりません</b>（カーソル直前から続く半角の英数字・記号だけが対象）。</span></li>
    <li><b>動作の速さ</b><span>「安定重視／標準（既定）／最速」から選びます。変換に失敗する、途中で崩れるといった場合は「安定重視」にしてください。速いほど処理は短く済みますが、対象のアプリが追いつかずに失敗しやすくなります。</span></li>
    <li><b>待機時間と上限</b><span>速さの設定で決まる値が一覧で見えます。個別に変えると「カスタム」になります。<b>キーボード保護の上限</b>（既定 30 秒）を超えると、キーを押して処理を中止できるようになります。それまでは押しても何も起きません（キー入力は処理が終わるまで止めたままです）。<b>選択確認の上限</b>（既定 5 秒）は、打ち直す文字を選び直せたかの確認を諦めるまでの時間です。</span></li>
    <li><b>クリップボード</b><span>処理中に一時的に上書きされる内容の扱いを選びます。そのまま保持（既定）／元の内容を復元／空にする。<b>既定のままにしておくと、誤変換や中断のときに <code>Ctrl+V</code> で元のローマ字へ戻せます。</b></span></li>
...
目安は**ローマ字で 150 文字程度**です。ただし文の区切り方によって前後します。下の例文は 147 文字です。

```
otukaresamadesukyounokaigidehaatarasiikinounosekkeinituitegironsimasitagajikangatarinakunattanodetuzukiharaisyuunouchiawasedekimerukotoninarimasita
→ お疲れさまです今日の会議では新しい機能の設計について議論しましたが時間が足りなくなったので続きは来週の打ち合わせで決めることになりました
```

**もう一つの問題は処理時間です。** 1 文字につきキーを 2 回送るため（読み取る範囲を選ぶときと、打ち直すとき）、所要時間は文字数に比例して伸びます。**設定を最速にしても縮まない下限**があります。
...
**実際にはこれより長くかかります。** 上の数値はキーを送る間隔だけを足したもので、実際の所要時間は**お使いのアプリと PC の重さに左右されます。** 手元の実測では、標準の設定で約 300 文字が 4.5〜4.9 秒でした（上の計算では 3.6 秒にあたる長さです）。動作の重いアプリでは、次の操作へ進む前の待ち時間がさらに効いてきます。選択の取りこぼしで自動再試行が入るとやり直しになり、安定重視で 800 文字の場合は約 19 秒かかった実測例があります。同じ文字数・同じ設定でも、そのときの状況で時間は変わります。

**この間はキーボードの入力を止めています。** 打った文字が変換中の文章へ混ざらないようにするためで、長い文章ほど「キーが効かない時間」が延びます。処理中にうっかりクリックして中断してしまう余地も広がります。

**文や文節の区切りごとに変換してください。** 変換候補を選び直せる状態も保てます。
