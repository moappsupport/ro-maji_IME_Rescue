...
- **設定項目「コピー完了待機」を「読み取り上限」に変更しました**
  ...
  効く場面が変わったため名前も変えています。この設定を調整されていた場合は既定値に戻ります。

- **通知メッセージを原因に応じた文言にしました**

  「選択状態を確認できませんでした」が原因によらず同じ案内文でしたが、原因ごとの対処法を表示します。

- **設定の保存を安全にし、既定の待機時間を見直しました**

  保存中に電源が切れても設定ファイルが壊れにくくなりました。「最速 / 標準 / 安定重視」の待機時間も実測にもとづいて調整しています（同じプリセットをお使いなら新しい値へ引き継がれます）。
