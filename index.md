...
複数行にわたってローマ字を打ってしまった場合、変換されるのはカーソルがある行だけです。それより前の行は残ります。

```
kyouhaiitenki           ← 変換されません
desunodesannposisimasu  ← ここにカーソルがあると、この行だけ変換されます
```

改行をまたいで変換することはできません。**行ごとにカーソルを置いて変換してください。**
...
```
testnokakikata   → test ごと変換されてしまいます
test nokakikata  → nokakikata だけが変換されます
```
...
`HTML` `OK` のように**大文字が 2 文字以上続く部分は、そのままの表記で残ります**。

```
HTMLnokakikata        → HTMLのかきかた
kyouhaHTMLwobenkyou   → きょうはHTMLをべんきょう
```

一方、`test` のような**小文字の英単語はローマ字と区別できない**ため、かなに変換されます。大文字が 1 文字だけの場合も小文字になります。残したい場合は、間に半角スペースを入れてから変換してください。
