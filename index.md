...
- **設定画面を見直しました**
  ...
  待機時間の一覧を折りたたまず常に表示します。これまでは「詳細設定」を開かないと存在に気付けませんでした。あわせて文字を読みやすい大きさに調整しています。
...
目安は**ローマ字で 150 文字程度**です。実測では 150 文字までは確定されず、160 文字を超えたあたりから始まりました。ただし文の区切り方によって前後します。下は 147 文字の例で、この長さがひとつの上限の感覚です。
...
**実際にはこれより長くかかります。** 上の数値はキーを送る間隔だけを足したもので、実際の所要時間は**お使いのアプリと PC の重さに左右されます。** 手元の実測では、標準の設定で約 300 文字が 4.5〜4.9 秒でした（上の計算では 3.6 秒にあたる長さです）。同じ条件で繰り返しても 0.4 秒ほど幅が出ます。動作の重いアプリでは、次の操作へ進む前の待ち時間がさらに効いてきます。選択の取りこぼしで自動再試行が入るとやり直しになり、安定重視で 800 文字の場合は約 19 秒かかった実測例があります。同じ文字数・同じ設定でも、そのときの状況で時間は変わります。

**この間はキーボードの入力を止めています。** 打った文字が変換中の文章へ混ざらないようにするためで、長い文章ほど「キーが効かない時間」が延びます。処理中にうっかりクリックして中断してしまう余地も広がります。

速さの設定で解決する問題ではありません。**文や文節の区切りごとに変換してください。** 変換候補を選び直せる状態も保てます。
